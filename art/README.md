...
art/
  ...
  icon/icon.png              The launcher icon at 1×, source of the store's 512×512.
...
tools/
  ...
  gen_sprites.py             Generates the whole of art/sprites and art/preview.
  preview_scene.py           Assembles the test scene.
  verify_assets.py           Checks the acceptance criteria.
```

`art/` also holds three things this brief never asked for, generated by their own
scripts and documented elsewhere. They are listed here only so that the tree
above is not read as the whole directory:

```
art/sfx/*.wav                Four sound effects — tools/gen_sfx.py, spec §13b.
art/music/theme.wav          The looping background theme — tools/gen_music.py.
art/store/                   Play listing graphics — tools/gen_store.py, docs/PLAY_STORE.md.
tools/audio.py               Synthesis primitives, to the audio what ega.py is to the art.
```

None of them is covered by `verify_assets.py`, and deliberately so: the store
graphics are pre-scaled on purpose, which hard rule 3 forbids everywhere else,
and the audio has no pixels to check.
