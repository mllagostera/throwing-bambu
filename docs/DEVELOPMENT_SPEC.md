...
Four effects (T-50), generated by `tools/gen_sfx.py` into `art/sfx/` and packaged from there by the same Gradle task that carries the sprites. §18 forbids reusing the original's assets and the store listing promises original work, so nothing here is a downloaded sample; they are synthesised from arithmetic, with every random choice from a fixed-seed LCG, so regenerating is byte-identical exactly as the art is.

22 050 Hz, mono, 16-bit PCM WAV — `SoundPool` decodes that natively, so the build transcodes nothing.

| File | When | Length |
|---|---|---|
| `throw.wav` | `ShotFired`, with the throwing pose | 180 ms |
| `boom.wav` | on a hit, with explosion **frame 0** | 550 ms |
| `victory.wav` | `MatchEnd`, winner is a local seat | 420 ms |
| `defeat.wav` | `MatchEnd`, winner is not | 540 ms |

**`SoundPool`, not `MediaPlayer`.** Short clips that must start on the frame they are asked for, may overlap, and repeat all match. Three streams: a throw's tail can still be sounding when the blast starts.

**The blast plays on frame 0, not on the crater's frame 3.** The sound belongs to the flash, and 120 ms late is far enough to hear as a mistake — which is the opposite of the reason the *crater* waits for frame 3.

**Won or lost is a property of who is watching**, not of the match. The winner is checked against `humanPlayers`, so a hot-seat match always hears the victory: somebody at that device did win.

**Silent mode is read at play time**, never cached — a phone can be silenced mid-match. Vibrate counts as silent. The attributes are `USAGE_GAME`, so the effects sit on the media stream the volume keys reach during play.

**The bank belongs to the screen, not the view model.** A `SoundPool` holds decoder threads and must be released, and only the composition knows when the screen is gone; the view model deliberately survives a rotation (T-21), so it borrows the bank and hands it back on dispose rather than coming out of a rotation holding a released pool.

**The load listener is installed before the first `load`.** `SoundPool` reports completion on a worker through a single listener and a completion that arrives before the listener exists is dropped, not queued. These files are small enough to lose that race, and losing it leaves every sound permanently unready and the game permanently silent.

Loading is asynchronous and nothing waits for it: an effect asked for in the first moments after the screen opens is silent rather than stalling the animation. A missing file costs that one effect, not the game.

### The background theme

`art/music/theme.wav`, from `tools/gen_music.py`: 16 bars at 92 BPM with swung eighths, about 42 seconds, looping. Walking bass, electric-piano comping off the beat, a sparse lead and brushes, mixed at 0.32 against the effects' 0.85 — it is furniture, and must never be why a throw is hard to hear.

An **original tune in a borrowed idiom**. The harmony is a ii-V-I with turnarounds, which nobody owns; the melody is written for this game. The obvious reference is a specific composition with a named author, and §18 draws that line for `GORILLA.BAS` already: the idiom is free, the work is not.

**`MediaPlayer`, not `SoundPool`** — the opposite call to the effects', for the opposite reason. `SoundPool` decodes whole samples into memory for short clips fired repeatedly; this is a 1.8 MB file played once and looped forever. Looping is `isLooping`, not a completion listener restarting it: the gap a restart leaves is small, audible, and lands in the same place every 42 seconds.

**It follows the lifecycle, not the composition.** `ON_PAUSE` stops it and `ON_RESUME` picks it up where it left off, so the tune does not play from the recents screen or over a phone call. It lives at the root of `BambuApp` so navigating does not restart it.

### The two switches (`app/settings/AudioSettings.kt`)

Music and effects are turned off **separately**, from the settings screen, and the choice survives the app. They fail differently: the music is what somebody turns off to play with their own on, the effects are what they turn off in a waiting room. One control means whoever wants either has to give up both.

`SharedPreferences`, like the language and in the same file, for the reason [`LocaleStore`] gives and one more: the value has to be known before the first composition, or the music starts and is then cut off — the same fault as the language's blink, with a symptom nobody misses.

Written with `commit` rather than `apply`, which is where this departs from the language. `apply` schedules the write and returns, so a process killed before the flush loses it; that is not theoretical, it was observed. A toggle is a rare deliberate tap on a file of two booleans, so synchronous I/O costs nothing anybody can feel.

The values are Compose state published through `LocalAudioSettings`, so the screen, the bank and the player read one truth and a toggle takes effect on the frame it is made. Silent mode is enforced independently: the switches say what the player wants, the ringer says what the room allows, and either one is enough to stay quiet.

### Why the audio is stored uncompressed

The WAVs cost their full size in the APK — about 1.9 MB, nearly all of it the theme — because AAPT stores them rather than deflating them. That is a requirement, not an oversight: `AssetManager.openFd`, which both the bank and the player need in order to hand a file descriptor to the platform, fails on a compressed asset. Shrinking the audio means a shorter loop or a lower sample rate, not a better packer.
