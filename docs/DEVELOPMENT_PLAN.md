...
| ✅ T-50 | Audio: throw, explosion, victory, defeat; `SoundPool`, respects silent mode. Generated by `tools/gen_sfx.py` (§13b); the settings toggle is T-51's half | 0.6 d |
| ✅ T-50b | Background theme: a looping lounge tune from `tools/gen_music.py`, `MediaPlayer`, tied to the lifecycle, silent mode respected. Not in the original plan — asked for after the effects landed | 0.3 d |
| 🟡 T-51 | Persistent settings: **sound and music done**, as two separate switches in `AudioSettings` (§13b) — `SharedPreferences` beside the language, not DataStore, because the value is needed before the first composition. `speedMultiplier`, default AI level and rounds to win remain | 0.4 d |
